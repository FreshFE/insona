...
//获取对象
	...
	button.className="Slideshow_button";
	for(var i=0;i<li.length;i++){
		...
		a.className="b1";a.onmouseover=function(){
			this.className="Slideshow_b2"
		};
		a.onmouseout=function(){
			this.className="Slideshow_b1";sc(j)
		};
...
//控制焦点按钮
	function sc(n){
		for(var i=0;i<li.length;i++){
			button.childNodes[i].className="Slideshow_b1"
			};
			button.childNodes[n].className="Slideshow_b2";
		}
		title.className="Slideshow_num_list";
		title.appendChild(span);
		alpha(title,opac1);
		id.className="Slideshow_d1";
		div.className="Slideshow_d2";
		id.appendChild(div);
...
//渐隐
	var fadeout=function(){
		opac-=5;div.innerHTML=li[j].innerHTML;
		alpha(div,opac);if(scton==0){num=2;scrolltxt();
		scton=1};if(opac>0){
